    ...
    def run(self):
        ...
        try:
            ...
            if not MIGRATION_CONFIG.dry_run:
                ...
                logger.info("\n[STEP 5] Skipping clear (dry run mode)")
            ...
            if not MIGRATION_CONFIG.dry_run:
                ...
                logger.info("\n[STEP 8] Skipping cleanup (dry run mode)")
            
            # Step 9: Set Alembic version
            if not MIGRATION_CONFIG.dry_run and MIGRATION_CONFIG.set_alembic_version:
            ...
            elif MIGRATION_CONFIG.dry_run:
                logger.info("\n[STEP 9] Skipping Alembic version update (dry run mode)")
            else:
            ...
            if not MIGRATION_CONFIG.dry_run:
                ...
                logger.info("\n[STEP 10] Skipping default settings insertion (dry run mode)")
            ...
            if not MIGRATION_CONFIG.dry_run:
                ...
                logger.info("\n[STEP 11] Skipping schema changes (dry run mode)")
    ...
    def _migrate_table(
        ...
        try:
            ...
            if not MIGRATION_CONFIG.dry_run:
                ...
                logger.info(f"  [DRY RUN] Would load {len(final_rows)} rows")
                self.statistics['table_stats'][target_table] = {
                    'source_rows': len(source_rows),
                    'would_migrate': len(final_rows),
                    'duration': time.time() - table_start
                }
    ...
    def _print_summary(self):
        ...
        if self.statistics['table_stats']:
            ...
            for table, stats in self.statistics['table_stats'].items():
                ...
                elif 'would_migrate' in stats:
                    print(f"  {table}: {stats['source_rows']} -> {stats['would_migrate']} (dry run)")
                else:
                    print(
...
def main():
    ...
    parser.add_argument(
        '--dry-run',
        action='store_true',
        help='Run migration without actually writing data'
    )
    parser.add_argument(
        '--log-level',
        type=str,
